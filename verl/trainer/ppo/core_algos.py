...
@register_adv_est(AdvantageEstimator.OPD)  # Online Policy Distillation
def compute_opd_advantage(
    ...
    **kwargs,
    ...
    """
    Compute advantage for Online Policy Distillation (OPD).
    
    Unlike GRPO which uses group-based outcome rewards, OPD computes per-token advantages
    from reverse KL divergence between student and teacher log probabilities.
    
    The method expects teacher_log_probs to be passed in kwargs and computes:
        reverse_KL = log π_student - log π_teacher
        advantages = -reverse_KL
    
    This provides dense (per-token) supervision while maintaining on-policy sampling.
    
    Reference: https://thinkingmachineslab.com/blog/on-policy-distillation
    
    Args:
        token_level_rewards: (bs, response_length) - Not used in OPD, but kept for compatibility
        response_mask: (bs, response_length) - Mask indicating valid tokens
        index: (bs,) - Group ID per sample (not used in OPD but kept for compatibility)
        epsilon: float for numerical stability
        norm_adv_by_std_in_grpo: bool - Whether to normalize advantages (typically False for OPD)
        config: (AlgoConfig) algorithm settings
        **kwargs: Must contain 'teacher_log_probs' and 'student_log_probs'
    
    Returns:
        advantages: (bs, response_length) - Per-token advantages from reverse KL
        returns: (bs, response_length) - Same as advantages for OPD
    
    Raises:
        ValueError: If teacher_log_probs or student_log_probs not provided in kwargs
    """
    # Extract teacher and student log probs from kwargs
    if "teacher_log_probs" not in kwargs:
        raise ValueError("OPD requires 'teacher_log_probs' in kwargs. "
                        "Make sure teacher logprobs are computed and passed to compute_advantage.")
    if "student_log_probs" not in kwargs:
        raise ValueError("OPD requires 'student_log_probs' in kwargs. "
                        "Make sure student (rollout) logprobs are available.")
    
    teacher_log_probs = kwargs["teacher_log_probs"]  # (bs, response_length)
    student_log_probs = kwargs["student_log_probs"]  # (bs, response_length)
    
    # Check if text splitting is enabled for cross-tokenizer distillation
    use_text_splits = config is not None and config.get("opd_text_splitter", None) is not None
    
    # Compute reverse KL divergence per token (or per split)
    # reverse_KL = log π_student - log π_teacher
    with torch.no_grad():
        if use_text_splits:
            # Use text splitting for cross-tokenizer compatibility
            from verl.trainer.ppo.text_splitter import (
                aggregate_logprobs_by_splits, broadcast_split_values_to_tokens,
                get_text_splitter)
            
            splitter_config = config.get("opd_text_splitter", {})
            splitter_name = splitter_config.get("name", "word")
            aggregation = splitter_config.get("aggregation", "mean")
            
            # Get responses, tokenizers, and texts from kwargs
            if "responses" not in kwargs or "tokenizer" not in kwargs:
                raise ValueError(
                    "OPD with text splitting requires 'responses' and 'tokenizer' in kwargs"
                )
            
            responses = kwargs["responses"]  # (bs, response_length)
            tokenizer = kwargs["tokenizer"]
            teacher_tokenizer = kwargs.get("teacher_tokenizer", tokenizer)
            
            # Initialize splitter
            splitter = get_text_splitter(splitter_name, **splitter_config.get("params", {}))
            
            # Process each sequence in the batch
            advantages = torch.zeros_like(student_log_probs)
            
            for seq_idx in range(student_log_probs.shape[0]):
                # Get valid tokens (masked)
                valid_mask = response_mask[seq_idx] > 0
                valid_tokens = responses[seq_idx][valid_mask].tolist()
                ...
                # Split text
                try:
                    student_split_ids, _ = splitter.map_tokens_to_splits(
                        valid_tokens, text, tokenizer
                    )
                    teacher_split_ids, _ = splitter.map_tokens_to_splits(
                        valid_tokens, text, teacher_tokenizer
                    )
                    
                    num_splits = max(max(student_split_ids, default=0), max(teacher_split_ids, default=0)) + 1
                    
                    # Aggregate logprobs to split level
                    student_logprobs_valid = student_log_probs[seq_idx][valid_mask]
                    teacher_logprobs_valid = teacher_log_probs[seq_idx][valid_mask]
                    
                    student_split_logprobs = aggregate_logprobs_by_splits(
                        student_logprobs_valid, student_split_ids, num_splits, aggregation
                    )
                    teacher_split_logprobs = aggregate_logprobs_by_splits(
                        teacher_logprobs_valid, teacher_split_ids, num_splits, aggregation
                    )
                    
                    # Compute reverse KL at split level
                    ...
                    token_advantages = broadcast_split_values_to_tokens(
                        split_advantages, student_split_ids, len(valid_tokens)
                    )
                    
                    # Place back in full sequence
                    advantages[seq_idx][valid_mask] = token_advantages
                    
                except Exception as e:
                    # Fall back to token-level KL if splitting fails
                    print(f"Warning: Text splitting failed for sequence {seq_idx}: {e}. Falling back to token-level KL.")
                    advantages[seq_idx] = -(student_log_probs[seq_idx] - teacher_log_probs[seq_idx])
        else:
            # Standard token-level reverse KL (same tokenizer)
            reverse_kl = student_log_probs - teacher_log_probs  # (bs, response_length)
            
            # Use -reverse_KL as advantages
            # Minimizing this pushes student to behave like teacher
            advantages = -reverse_kl
        
        # Optional: normalize advantages by std (typically not used in OPD)
        if norm_adv_by_std_in_grpo and config is not None:
            # Normalize per group if needed
            ...
            for i in range(bsz):
                # Compute mean advantage per sequence (across valid tokens)
                valid_mask = response_mask[i] > 0
                if valid_mask.sum() > 0:
                    seq_advantage = advantages[i][valid_mask].mean()
                    id2advantages[index[i]].append(seq_advantage)
            
            # Compute group statistics
            for idx in id2advantages:
                if len(id2advantages[idx]) > 1:
                    advs_tensor = torch.stack(id2advantages[idx])
                    id2mean[idx] = torch.mean(advs_tensor)
                    id2std[idx] = torch.std(advs_tensor)
                else:
                    id2mean[idx] = torch.tensor(0.0)
                    id2std[idx] = torch.tensor(1.0)
