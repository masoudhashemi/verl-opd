...
def compute_teacher_log_probs(
    data: DataProto,
    teacher_wg,
    ref_in_actor: bool = False,
) -> DataProto:
    """
    Compute teacher log probabilities for OPD (Online Policy Distillation).
    
    This function computes the teacher's log probabilities for the student's generated tokens.
    The teacher can be:
    - A separate larger model (launched as reference policy worker)
    - The reference model itself
    
    Args:
        data: DataProto containing the student's rollout data
        teacher_wg: Teacher worker group (can be ref_policy_wg or actor_rollout_wg)
        ref_in_actor: Whether the teacher is in the actor worker group
    
    Returns:
        DataProto containing teacher_log_probs
    """
    if teacher_wg is None:
    ...
    if "ref_log_probs" in teacher_output.batch:
        teacher_output.batch["teacher_log_probs"] = teacher_output.batch.pop("ref_log_probs")
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # For OPD, pass teacher and student log probs
        if adv_estimator == core_algos.AdvantageEstimator.OPD:
            ...
            # For text splitting (cross-tokenizer distillation)
            if config and config.get("opd_text_splitter", None) is not None:
                if "responses" in data.batch:
                    adv_kwargs["responses"] = data.batch["responses"]
                if "tokenizer" in data.meta_info:
                    adv_kwargs["tokenizer"] = data.meta_info["tokenizer"]
                if "teacher_tokenizer" in data.meta_info:
                    adv_kwargs["teacher_tokenizer"] = data.meta_info["teacher_tokenizer"]
                    ...
                    if self.config.algorithm.adv_estimator == "opd":
                        with marked_timer("teacher_log_prob", timing_raw, color="purple"):
                            ...
                            teacher_log_probs = compute_teacher_log_probs(
                                batch, 
                                teacher_wg=self.ref_policy_wg if not self.ref_in_actor else self.actor_rollout_wg,
